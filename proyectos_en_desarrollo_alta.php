...
//En caso de recibir actualizacion, actualizamos los datos del registro 
		...
		$sql_update="UPDATE proyectos SET pelicula='".$_POST['pelicula']."',director='".$_POST['director']."' ,eficine='".$_POST['eficine']."' ,periodo='".$_POST['periodo']."' ,genero='".$_POST['genero']."',notas='".$notas."".$_POST['notas_pasadas']."'  WHERE id=".$_POST['id']."";
...
if($_POST['efiyear']){ 
		...
		$sql="INSERT INTO proyectos(pelicula,eficine,periodo,genero) values('".$_POST['pelicula']."','".$_POST['efiyear']."','".$_POST['periodo']."','0')";
	$mysqli->set_charset("utf8");	
...
if($_POST['items_length']>0){
	...
	//Extraemos el id recien creado para esta nueva pelicula de la tabla de peliculas
		$sql_2="SELECT id FROM `proyectos` WHERE pelicula='".$_POST['pelicula']."'";	
		...
		for ($i = 0; $i < $_POST['items_length']; $i++) {
			...
			$cantidad="cantidad".$i;
    				 	...
    				 	$sql_3="INSERT INTO prospectos(id_p,id_e,aportacion) values(".$row2['id'].",".$id_e.",".$cantidad.")";
...
$sql_p="SELECT id, pelicula, eficine, genero FROM `proyectos` ORDER BY `eficine` DESC ";
						...
						if($cont_p%2==0)
							...
							<tr id='".$cont_p."' ".$color.">
								...
								<td id='".$row_p['pelicula']."' style='padding:0px 5px;;' >
									<a href='editar_proyecto.php?id=".$row_p['id']."&email=".urlencode($_GET['email'])."'>
...
<div id="locate" style="position:fixed;  z-index:1000; top:0px;  height:35px; width:625px; margin:0 auto; background: #888 ; margin-bottom:80px;"> <span style="position:relative; float:right; color:#FFFFFF; margin:10px; margin-top:3px; font-size:24px">Proyectos en desarrollo</span> </div>
    ...
    <div id="confirmacion" style="position:fixed;  z-index:1000; top:35px; height:50px;width:625px; margin:0 auto;  background:#fff; margin-bottom:80px;">
        ...
        <a href="majorsforyear.php?email=<? echo urlencode($_GET['email']);?>" target="_blank"  ><button class="boton-superior" style="cursor:pointer">Jos&eacute;</button></a>
	...
	<form  action="proyectos_en_desarrollo_alta.php?email=<? echo urlencode($_GET['email']);?>" method="post" name="agregar" id="agregar" enctype="application/x-www-form-urlencoded" >
