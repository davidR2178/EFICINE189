...
<body style="font-family:Gotham, 'Helvetica Neue', Helvetica, Arial, sans-serif">
    ...
    <div id="confirmacion" style="position:fixed;  z-index:1000; top:0px; left:0px; right:0px; height:7%; background:#999; margin-bottom:80px;">
        ...
        <a href="basura.php?email=<?php echo urlencode($_GET['email']);?>" target="_blank" ><button class="boton-superior" style="cursor:pointer">Basura</button>	</a>
...
//Averiguamos la sesion guardada para el usuario   
   	...
   	if($resultw=$mysqli->query($sql_1)){
	
		echo(session_id()."</br>");
		$resultw_dump = $resultw->fetch_array(MYSQLI_ASSOC);
		echo ($resultw_dump['phpsessid']);
		printf("Errormessage: %s\n", $mysqli->error);}
//Si la sesión guardada es la sesión activa, entonces damos acceso a la lista y sus funciones
      
	if($resultw_dump['phpsessid']==session_id()){ 

		
	printf($resultw_dump['phpsessid']); 
	//Para desplegar la lista en caso de no que no se tenga una nueva o se tenga que actualizar alguna empresa
	//Averiguar si estas variables se pueden tomar de más arriba
	if($mail=="david@avantipictures.com")
		{
			...
			$name="Rodrigo";
  						...
  						//Este es el query que extrae de la tabla de aportaciones LOS id_e DE LAS EMPRESAS QUE HAN DADO APORTACIONES
						$sql_b="SELECT DISTINCT `id_e` FROM `aportaciones` ORDER BY id_e ASC ";
						//Se extraen de 'aportaciones' los id_e de las empresas que han dado aportaciones, el DISTINCT evita que el id_e se repita, ya que en la tabla de aportaciones una misma empresa ha dado diferentes aportaciones y por lo tanto tiene varios registros en la tabla
						...
						$rowf=array();//En este array se guardan los nombres de las empresas
						
						$cont=0;
						$contb=0;
						$total=0;
						
						//Para cada id_e hacer lo siguiente
						while ($rowz = $result->fetch_array(MYSQLI_ASSOC)) {
							...
							//Se ordenan estos datos en el array asociativo $row
							$row = $resultz->fetch_array(MYSQLI_ASSOC);
							if($row['prioridad']!='0'){
										$color="background-color:#".$row['prioridad']."; position:relative;height:30px;'";
									}
									else{	
										if($cont%2==0){
											$color="background-color:#fff; position:relative;  height:50px;";
										}
										else{
											$color="background-color:#D8D8D8; position:relative; height:50px;";
										}; 
									}		
							//Se almacenanlos datos en el array $rowf, aqui estan todas las empresas que han aportado
							...
							//Para cada aportacion segun $result_aportacion hacer lo siguinete
							while($row_aportacion=$result_aportacion->fetch_array(MYSQLI_ASSOC)){
								    //Hacer SUMATORIA POR EMPRESA de sus aportaciones
									$total= $total + $row_aportacion['aportacion'];
									}
							//sumamos a $efi_total el total aportado de cada empresa	y damos formato al numero final	
							$efi_total=$efi_total + $total;
							$efi_total_final=$efi_total;
							//Vamos insertando registros en el array $conjunto2
							$conjunto2[$row['empresa']]=$total;
							//damos formato a la cantidad aportada por cada empresa desde 2006
							setlocale(LC_MONETARY, 'es_MX'); 
							$total_aportacion=$total;	
							
							
							
							//Insertamos los registros en el array $conjunto, notese que el KEY esta dado por la variable $total, de eta manera tenemos la posibilidad de ordenar el array de acuerdo a un numero
							//puede que haya algunos totales que sean iguales y anulen la clave o la encimen y por eso faltan alguans empresas
							
							array_push($conjunto_bis,$row['empresa']);
							
							
							foreach($conjunto as $key => $valor){
								if($conjunto[$key]==$conjunto[$total]){
									$total++;}
								}
							...
							$conjunto[$total]="
								...
								<td id='".$row['empresa']."' style='padding:0px 5px; width=300px;".$color."' >
									<a href='editar_empresa_publicidad.php?id=".$row['id']."&email=".urlencode($_GET['email'])."'>
								...
								<td style='text-align:right; min-width:100px;".$color."' id=''>".$total_aportacion."</td>";
							//reiniciamos el valor de la variable $total, con el fin de que la proxima iteracion vuelva a empezar	
							$total=0;
							$contb++;
...
<div id="locate" style="position:fixed;  z-index:1000; top:0px;  height:35px; width:625px; margin:0 auto; background: #888 ; margin-bottom:80px;"> 
	<span style="position:relative; float:right; color:#FFFFFF; margin:10px; margin-top:3px; font-size:24px">Contribuyentes</span> 
</div>
    <div id="confirmacion" style="position:fixed;  z-index:1000; top:35px; height:100px;width:625px; margin:0 auto;  background:#fff; margin-bottom:80px;">
        ...
        <a href="majorsforyear.php?email=<?php echo urlencode($_GET['email']);?>&year=2014"  ><button class="boton-superior" style="cursor:pointer">2014</button></a>
    </div>
    <a id="arriba"></a>  
     ...
     <div style="position:relative; margin-top:145px; width:650px; ">
        <TABLE class="todas-empresas" border="0" bordercolor="#CCCCCC" style="position:relative; left:-10px;" >
         ...
         ?>
         	<tr style='padding:0px 5px; width=300px;'><td  style='padding:0px 5px; width=300px;'></td><td >Aportacion total</td><td style='text-align:right; min-width:100px;'><?php echo $efi_total_final?></td>
            </tr>
         </tbody>
        </TABLE>
...
//En caso de que la sesion guardada no coincida con la sesión activa
else{echo $result_dump['phpsessid']; }
