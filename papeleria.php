...
//Se extraen sólo los registros asignados al usuario de la sesion y que no han sido despuplicados	
	$sql_b="SELECT id, empresa, prioridad FROM `publicidad` WHERE uid=".$uid." AND unpublish=0 ";
...
function unpublish(id){ 
	...
	var xmlhttp;
		...
		xmlhttp.open("GET","unpublish_papeleria.php?id="+id,true);
...
.boton-superior{ position:relative; float:left; width:20%; height:100%; background-color:#CCFF33; color:#0000FF; font-size:40px;}
...
<body style="font-family:Gotham, 'Helvetica Neue', Helvetica, Arial, sans-serif">
    ...
    <div id="confirmacion" style="position:fixed;  z-index:1000; top:0px; left:0px; right:0px; height:7%; background:#999; margin-bottom:80px;">
        ...
        <? }
		?>
        <a href="publicidad.php?email=<? echo urlencode($_GET['email']);?>" target="_blank" ><button class="boton-superior" style="cursor:pointer">EFICINE 189</button>	</a>
...
//----------------------------------------------------------------------------------------------------------------------
// Actualización desde editar_empresa_papeleria.php -----------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------

   
	//En caso de recibir actualizacion de ulguna empresa desde editar_empresa_papeleria.php (hay que checar si es posible no volver a conectar a la db)
	if($_POST['id']){
...
//Actualizamos los datos del registro (a partir de aqui entra en accion las nuevas bases de datos)
	...
	$sql_notas="UPDATE notas SET papeleria='".$notas."".$_POST['notas_pasadas']."' WHERE id=".$_POST['id']."";
...
//Aqui ya se extraen datos de las nuevas bases de datos	
	$sql_b="SELECT id, empresa FROM `empresas` WHERE uid=".$uid." AND papeleria=1 ";
						...
						while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
							...
							$sql_prioridad="SELECT papeleria FROM `prioridades` WHERE id=".$row['id']."";
							$result_prioridad=mysql_query($sql_prioridad);
							$row_prioridad=mysql_fetch_array($result_prioridad, MYSQL_ASSOC);
							
						if($row_prioridad['papeleria']!='0'){$color="style='background-color:#".$row_prioridad['papeleria'].";'";}
						...
						// En cada iteracion se extran los contactos de la empresa en turno
							$sql_contactos_b="SELECT id_contacto,contacto,puesto,telefono,ext,mail FROM `contactos` WHERE id=".$row['id']." AND papeleria=1";
						...
						// En cada iteracion se extran las notas de la lista papeleria de la empresa en turno
							$sql_notas="SELECT papeleria FROM `notas` WHERE id=".$row['id']."";
							...
							<tr id='".$cont."' ".$color.">
								...
								<td id='".$row['empresa']."' style='padding:0px 5px;;' >
									<a href='editar_empresa_papeleria.php?id=".$row['id']."&email=".urlencode($_GET['email'])."'>
								...
								<td>
									<div style='height:100px; overflow:auto;'>".$row_notas['papeleria']."</div>
								</td>
								<td>
...
<head>
<meta http-equiv="content-type" content="text/html" charset="UTF-8">
<title><? echo $name?> | Alianzas	</title>
...
<body style="font-family:Gotham, 'Helvetica Neue', Helvetica, Arial, sans-serif">
	<div id="locate" style="position:fixed;  z-index:1000; top:0px; left:0px; right:0px; height:35px; background: #888 ; margin-bottom:80px;"> <span style="position:relative; float:right; color:#FFFFFF; margin:10px; margin-top:3px; font-size:24px">Alianzas</span> </div>
