...
//---------------------------------------------------------------------------------------------------------------------------------------------							
			...
			//Este query especifica los id de las obras en el año especificado en el GET
			$sql_b="SELECT id FROM `teatro` WHERE eficine='".$_GET['year']."'";		
		}
	  
						//Se extraen los id de las obras que han sido apoyadas en el año especificado en el GET segun eñ query $sql_b
...
//---------------------------------------------------------------------------------------------------------------------------------------------							
						...
						//ya tenemos las obras apoyadas durante el año seleccionado
						//necesitamos extrar los id de las empresas que apoyaron a esa pelis
						//Para cada obra del $result hacer lo siguiente:
						while ($rowz = mysql_fetch_array($result, MYSQL_ASSOC)) {																				
							//Se extraen de 'aportaciones_teatro' los id_e de las empresas que aportaron a la pelicula que se esta procesando y se ordenan de manera ascendente
							$sql_aportacion="SELECT id_e FROM `aportaciones_teatro` WHERE id_o=".$rowz['id']." ORDER BY id_e DESC";
							$result_aportacion=mysql_query($sql_aportacion);							
							//Se almacena el resultado de $result_aportacion en el array asociativo $rowd
							while ($rowd = mysql_fetch_array($result_aportacion, MYSQL_ASSOC)){
...
//---------------------------------------------------------------------------------------------------------------------------------------------							
						...
						$sql_all="SELECT DISTINCT id_e FROM `aportaciones_teatro` ORDER BY `id_e` DESC	";
...
//---------------------------------------------------------------------------------------------------------------------------------------------							
							...
							foreach($conjunto_all_ordenado as &$valor){
									...
									$result_not=mysql_query($sql_ide_not);
									//Se almacena el resultado de $resultz en el array asociativo $row
									$row_not = mysql_fetch_array($result_not, MYSQL_ASSOC);
...
<title>APORTANTES <? echo $_GET['year']?></title>
...
<body background="img/dark-triangles.png"  style="font-family: 'Lucida Grande', 'Lucida Sans Unicode', 'Lucida Sans', 'DejaVu Sans', Verdana, sans-serif; background-color:#999999">
    ...
    <? $var=json_encode($conjunto2);  ?>
        <div id="locate" style="position:fixed;  z-index:1000; top:0px;  height:35px; width:625px; margin:0 auto; background: #888 ; margin-bottom:80px;"> 
            <span style="position:relative; float:right; color:#FFFFFF; margin:10px; margin-top:3px; font-size:24px">Contribuyentes <? echo $_GET['year'] ?></span> 
        </div>
        ...
        <div style="position:relative; margin-top:145px; width:650px; ">
           ...
           <TABLE class="todas-empresas" border="0" bordercolor="#CCCCCC" style="position:relative; left:-10px;" >
                ...
                <tbody>
                <? foreach($conjuntob as $key=>$value)
                        {	
                            echo ($value);
                    };?>         	
               </tbody>
            </TABLE>
       </div>	<? print_r($conjunto_excluido);?>
       <script>
       if(screen.width<840){document.getElementByClass("todas_empresas").style.fontSize="24px";};
       </script>
       <? 	var_dump($conjunto3_ordenado);?>
    </div>
</body>
</html>   
<? }
