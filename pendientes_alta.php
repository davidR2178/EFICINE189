...
//En caso de recibir actualizacion, actualizamos los datos del registro 
		...
		$sql_update="UPDATE pendientes SET pelicula='".$_POST['pelicula']."',director='".$_POST['director']."' ,eficine='".$_POST['eficine']."' ,periodo='".$_POST['periodo']."' ,genero='".$_POST['genero']."',notas='".$notas."".$_POST['notas_pasadas']."'  WHERE id=".$_POST['id']."";
...
if($_POST['pelicula']){ 
		...
		$sql="INSERT INTO pendientes(pelicula,eficine,periodo,genero) values('".$_POST['pelicula']."','".$_POST['efiyear']."','".$_POST['periodo']."','0')";
	$mysqli->set_charset("utf8");	
...
if($_POST['items_length']>0){
	...
	//Extraemos el id recien creado para esta nueva pelicula de la tabla de peliculas
		$sql_2="SELECT id FROM `pendientes` WHERE pelicula='".$_POST['pelicula']."'";	
...
$sql_p="SELECT id, pelicula, eficine, genero FROM `pendientes` ORDER BY `eficine` DESC ";
...
<div id="locate" style="position:fixed;  z-index:1000; top:0px;  height:35px; width:625px; margin:0 auto; background: #888 ; margin-bottom:80px;"> <span style="position:relative; float:right; color:#FFFFFF; margin:10px; margin-top:3px; font-size:24px">Tareas pendientes</span> </div>
	...
	<form  action="proyectos_alta.php?email=<? echo urlencode($_GET['email']);?>" method="post" name="agregar" id="agregar" enctype="application/x-www-form-urlencoded" >
