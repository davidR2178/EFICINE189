...
$mysqli->set_charset("utf8");
   ...
   $sql_b="SELECT id, director, pelicula, eficine, periodo, genero, notas FROM `proyectos` WHERE id=".$_GET['id']."";
   
   $result=$mysqli->query($sql_b);
   $row = $result->fetch_array(MYSQLI_ASSOC);
   $mail=urldecode(($_GET['email']));

 $sql_c="SELECT id, id_e, aportacion FROM `prospectos` WHERE id_p=".$_GET['id']."";
...
<body background="img/dark-triangles.png"  style="font-family:Gotham, 'Helvetica Neue', Helvetica, Arial, sans-serif" bgcolor="#999999" >
	...
	<div style="width:100%; height:100%; background:none; position:absolute ">
		<form action="proyectos_en_desarrollo_alta.php?email=<? echo $mail;?>#<? echo $row['pelicula'] ?>" method="post" id="edit" name="edit">
    		<div style="width:625px; height:100%; background:#FFFFFF; position:relative; margin:-10px auto; top:0px;">
   				<div style="width:625px; height:80px; background:#<? if($row['genero']=="0") {echo 'd8d8d8';} else{echo $row['genero'];} ?>">
   					<input style="border:hidden; background:none; font-size: 26px; margin-top:25px; width:400px; margin-left:10px;" id="pelicula" name="pelicula" value='<? echo $row['pelicula']?>' />
    ...
    <button type="submit">Guardar</button>
                ...
                <div><table class="todas-empresas">
                   <thead>
                   <th>#</th><th>Contribuyente</th><th>Aportacion</th>
                   </thead>
                   <tbody>
                   <? while ($row_c = $result_c->fetch_array(MYSQLI_ASSOC)) {
                        ...
                        <tr>
                      		...
                      		<td>
                            		<a href="http://www.avantipictures.com/eficine/editar_empresa_publicidad.php?id=<? echo $row_c['id_e']?>&email=<? echo urlencode($_GET['email'])?>">
									<? echo $row_d['empresa']?>
                                </a>
                   
                   			</td>
                   			<td>
									<? setlocale(LC_MONETARY, 'es_MX'); echo money_format('%=-2#8.2n', $row_c['aportacion'])?>
                    			</td>
                   </tr>
	 ...
	 $contb++;}; ?>
      ...
      <form  action="editar_proyecto.php?email=<? echo urlencode($_GET['email']);?>&id=<? echo $_GET['id']?>" method="post" name="agregar" id="agregar" enctype="application/x-www-form-urlencoded" >
